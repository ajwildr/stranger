...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['action'])) {
        if ($_POST['action'] == 'create_task') {
            ...
            $insert_query = "
                INSERT INTO tasks (task_title, task_description, assigned_to, assigned_by, due_date, team_id, points, status) 
                VALUES (?, ?, ?, ?, ?, ?, ?, 'not_viewed')
            ";
            $stmt = $conn->prepare($insert_query);
            if ($stmt) {
                $stmt->bind_param("ssiisii", $task_title, $task_description, $assigned_to, $team_lead_id, $due_date, $team_id, $points);
                if ($stmt->execute()) {
...
// Fetch tasks assigned by the team lead
$tasks_query = "
    SELECT 
        t.task_id, 
        t.task_title, 
